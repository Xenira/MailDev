...
mailServer.create = function (port, host, user, password, secure, certFile, keyFile, hideExtensions) {
  ...
  if (secure) {
    ...
    if (certFile && keyFile) {
      smtpServerConfig.cert = certFile;
      smtpServerConfig.key = keyFile;
    } else {
      logger.warn(
        `Unable to load certificate files or no certificate files found. Using local certificates.\n` +
        `Define --ssl-key and --ssl-cert to supply custom certificates.`
      )
    }
  }
